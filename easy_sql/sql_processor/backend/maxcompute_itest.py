...
class MaxComputeTest(unittest.TestCase):
    ...
    def run_test_table(self, backend: MaxComputeBackend, table_name):
        ...
        self.assertEqual(table.first(),
                         MaxComputeRow.from_schema_meta(cols, types, (1, '1', dt('2020-01-01 11:11:11'))))
    ...
    def run_test_backend(self, backend: MaxComputeBackend, table_name):
        ...
        sc = table.schema
        self.assertListEqual(backend.exec_sql('select * from test_limit order by id').collect(), [
        ...
        self.assertListEqual(backend.exec_sql('select * from xx0 order by id').collect(), [
        ...
        self.assertRaisesRegex(Exception, re.compile(r'.* cannot save table.*'),
                               lambda: backend.save_table(TableMeta('test_limit'), TableMeta('xx'), SaveMode.overwrite,
                                                          False))
        backend.exec_native_sql('drop table if exists xx')

        # first save without partitions, should create
        backend.save_table(TableMeta('test_limit'), TableMeta('xx'), SaveMode.overwrite, True)
        self.assertListEqual(backend.exec_sql('select * from xx order by id').collect(), [
            MaxComputeRow(schema=sc, values=(1, '1')), MaxComputeRow(schema=sc, values=(2, '2'))
        ])
        ...
        backend.save_table(TableMeta('test_limit'),
                           ...
                           SaveMode.overwrite,
                           True)
        mc_row = partial(MaxComputeRow.from_schema_meta,
                         cols=['id', 'val'], types=['int', 'string'], pt_cols=['a'], pt_types=['string'])
        self.assertListEqual(backend.exec_sql('select * from xx order by id').collect(), [
            mc_row(values=(1, '1', dt('2021-01-01 00:00:00'))), mc_row(values=(2, '2', dt('2021-01-01 00:00:00'))),
        ])
        ...
        backend.save_table(TableMeta('test_limit'),
                           ...
                           SaveMode.overwrite,
                           True)
        self.assertListEqual(backend.exec_sql('select * from xx order by a, id').collect(), [
            mc_row(values=(1, '1', dt('2021-01-01 00:00:00'))), mc_row(values=(2, '2', dt('2021-01-01 00:00:00'))),
            mc_row(values=(1, '1', dt('2021-01-02 00:00:00'))), mc_row(values=(2, '2', dt('2021-01-02 00:00:00')))
        ])
        ...
        self.assertListEqual(backend.exec_sql('select * from xx order by a, id').collect(), [
            mc_row(values=(1, '1', dt('2021-01-01 00:00:00'))), mc_row(values=(2, '2', dt('2021-01-01 00:00:00'))),
            mc_row(values=(1, '1', dt('2021-01-02 00:00:00'))), mc_row(values=(1, '1', dt('2021-01-02 00:00:00'))),
            mc_row(values=(2, '2', dt('2021-01-02 00:00:00'))), mc_row(values=(2, '2', dt('2021-01-02 00:00:00')))
        ])
