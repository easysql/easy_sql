...
class PgSqlDialect(SqlDialect):
    ...
    def get_schemas_sql(self) -> str:
        return f"select schemaname from pg_catalog.pg_tables union " \
               f"select schemaname FROM pg_catalog.pg_views"
    ...
    def move_data_sql(self, target_table: str, temp_table: str,  partitions: List[Partition]) -> List[str]:
        ...
        for partition in partitions:
            ...
            sql.append(f'alter table {target_table} attach partition {target_partition_table_name} '
