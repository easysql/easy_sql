...
class ChSqlDialect(SqlDialect):
    ...
    def native_partitions_sql(self, table_name: str) -> Tuple[
        str, Callable[['sqlalchemy.engine.ResultProxy'], List[str]]]:
    ...
    def create_temp_table_with_schema_sql(self, temp_table_name: str):
        return f'create table if not exists ' \
               f'{temp_table_name} as {temp_table_name.split("__temp")[0]}'
    ...
    def move_data_sql(self, target_table_name: str, temp_table_name: str, partitions: List[Partition]):
        ...
        if len(partitions) != 0:
            self._check_partition_field_single(partitions)
            drop_pt_metadata_if_exist, insert_pt_metadata = self._generate_pt_metadata_sql(target_table_name,
                                                                                           partitions)
